...
<svg width="420" height="180" viewBox="0 0 420 180" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="title desc">
  ...
  <a href="https://hub.docker.com/u/${safeUsername}" target="_blank" rel="noopener"><text x="24" y="60" fill="${styleConfig.accent}" font-size="13" text-decoration="underline">@${safeUsername}</text></a>
  ${rows}
</svg>`;
}
...
/**
 * Create a repository-focused SVG card
 */
function createRepoStatsSvgWithStyle(style, data) {
  const styleConfig = getStyleConfig(style);
  const namespace = data.namespace || 'unknown';
  const repo = data.repo || data.name || 'unknown';
  const fullName = `${namespace}/${repo}`;
  const safeName = escapeXml(fullName);
  const pulls = data.pull_count ?? data.pullCount ?? 0;
  const stars = data.star_count ?? data.starCount ?? 0;
  const error = data.error || null;
  const safeError = error ? escapeXml(error) : '';
  const description = error
    ? `Unable to load Docker Hub stats for ${safeName}: ${safeError}`
    : `Docker Hub repository ${safeName}. Pulls ${formatNumber(pulls)}, stars ${formatNumber(stars)}.`;

  const rows = error
    ? `
      <rect x="24" y="76" width="372" height="78" rx="14" fill="${styleConfig.cardBg}" />
      <text x="40" y="108" fill="${styleConfig.errorColor}" font-size="14" font-weight="600">Unable to load repo</text>
      <text x="40" y="132" fill="${styleConfig.dimText}" font-size="13">${safeError}</text>
    `
    : `
      <g transform="translate(24 76)">
        <rect width="180" height="78" rx="14" fill="${styleConfig.cardBg}" />
        <text x="16" y="28" fill="${styleConfig.accent}" font-size="12">Pulls</text>
        <text x="16" y="56" fill="${styleConfig.text}" font-size="22" font-weight="700">${formatNumber(pulls)}</text>
      </g>
      <g transform="translate(220 76)">
        <rect width="176" height="78" rx="14" fill="${styleConfig.cardBg}" />
        <text x="16" y="28" fill="${styleConfig.accent}" font-size="12">Stars</text>
        <text x="16" y="56" fill="${styleConfig.text}" font-size="22" font-weight="700">${formatNumber(stars)}</text>
      </g>
    `;

  return `<?xml version="1.0" encoding="UTF-8"?>
<svg width="420" height="180" viewBox="0 0 420 180" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="title desc">
  <title id="title">Docker Hub Repository Stats</title>
  <desc id="desc">${description}</desc>
  <defs>
    <linearGradient id="bg" x1="16" y1="10" x2="404" y2="170" gradientUnits="userSpaceOnUse">
      <stop stop-color="${styleConfig.bg.start}" />
  ...
  <rect x="8" y="8" width="404" height="164" rx="20" fill="url(#bg)" />
  <rect x="8.5" y="8.5" width="403" height="163" rx="19.5" stroke="${styleConfig.border}" />
  <text x="24" y="38" fill="${styleConfig.text}" font-size="18" font-weight="700">Repository Stats</text>
  <a href="https://hub.docker.com/r/${safeName}" target="_blank" rel="noopener"><text x="24" y="60" fill="${styleConfig.accent}" font-size="13" text-decoration="underline">${safeName}</text></a>
  ${rows}
</svg>`;
}

/**
 * Create a pull-count trend sparkline card
 */
function createTrendSvgWithStyle(style, data) {
  const styleConfig = getStyleConfig(style);
  const username = data.username || `${data.namespace || 'unknown'}/${data.repo || 'unknown'}`;
  const safeName = escapeXml(username);
  const points = Array.isArray(data.points) ? data.points : [];
  const growth = data.growth || { pulls: 0, pullPercent: null };
  const days = data.days || 30;
  const error = data.error || null;
  const safeError = error ? escapeXml(error) : '';

  const values = points.map((point) => Number(point.totalPulls) || 0);
  const width = 420;
  const height = 180;
  const chartX = 24;
  const chartY = 78;
  ...
  if (values.length >= 2) {
    ...
    const coords = values.map((value, index) => {
      ...
      const y = chartY + chartH - ((value - min) / span) * chartH;
      return [x, y];
    });
    path = coords.map(([x, y], index) => `${index === 0 ? 'M' : 'L'}${x.toFixed(1)} ${y.toFixed(1)}`).join(' ');
    area = `${path} L${coords[coords.length - 1][0].toFixed(1)} ${(chartY + chartH).toFixed(1)} L${coords[0][0].toFixed(1)} ${(chartY + chartH).toFixed(1)} Z`;
  }

  const growthLabel = error
    ? safeError
    : values.length < 2
      ? 'Collecting history… check back tomorrow'
      : `${growth.pulls >= 0 ? '+' : ''}${formatNumber(growth.pulls)} pulls / ${days}d${growth.pullPercent == null ? '' : ` (${growth.pullPercent >= 0 ? '+' : ''}${growth.pullPercent}%)`}`;

  const growthColor = error
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-labelledby="title desc">
  <title id="title">Docker Hub Pull Trend</title>
  <desc id="desc">Pull trend for ${safeName}</desc>
  <defs>
    <linearGradient id="bg" x1="16" y1="10" x2="404" y2="170" gradientUnits="userSpaceOnUse">
      <stop stop-color="${styleConfig.bg.start}" />
      <stop offset="1" stop-color="${styleConfig.bg.end}" />
    </linearGradient>
    <linearGradient id="area" x1="0" y1="${chartY}" x2="0" y2="${chartY + chartH}" gradientUnits="userSpaceOnUse">
      <stop stop-color="${styleConfig.accent}" stop-opacity="0.35" />
      <stop offset="1" stop-color="${styleConfig.accent}" stop-opacity="0.02" />
    </linearGradient>
  </defs>
  <rect x="8" y="8" width="404" height="164" rx="20" fill="url(#bg)" />
  <rect x="8.5" y="8.5" width="403" height="163" rx="19.5" stroke="${styleConfig.border}" />
  <text x="24" y="38" fill="${styleConfig.text}" font-size="18" font-weight="700">Pull Trend</text>
  <text x="24" y="60" fill="${styleConfig.accent}" font-size="13">@${safeName}</text>
  <text x="24" y="168" fill="${growthColor}" font-size="12" font-weight="600">${escapeXml(growthLabel)}</text>
  ${area ? `<path d="${area}" fill="url(#area)" />` : ''}
  ${path ? `<path d="${path}" stroke="${styleConfig.accent}" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round" />` : `
    <rect x="${chartX}" y="${chartY}" width="${chartW}" height="${chartH}" rx="12" fill="${styleConfig.cardBg}" />
  `}
</svg>`;
}
