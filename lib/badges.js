...
function buildUserBadges(stats, baseUrl, options = {}) {
  ...
  const growthBadge = `${baseUrl}/api/badge/growth?username=${encodeURIComponent(username)}&days=${days}`;
  const cardUrl = `${baseUrl}/api/docker-stats?username=${encodeURIComponent(username)}&style=gradient`;
  const trendUrl = `${baseUrl}/api/trend?username=${encodeURIComponent(username)}&days=30&style=gradient`;
  ...
  return {
    ...
    badges: {
      ...
      growth: {
        endpoint: growthBadge,
        markdown: `![Docker Pull Growth](https://img.shields.io/endpoint?url=${encodeURIComponent(growthBadge)})`,
        days
      }
    },
    card: {
    ...
    trend: {
      url: trendUrl,
      markdown: `[![Docker Pull Trend](${trendUrl})](https://hub.docker.com/u/${username})`,
      html: `<a href="https://hub.docker.com/u/${username}"><img src="${trendUrl}" alt="Docker Hub pull trend for ${username}" /></a>`
    }
  };
}
